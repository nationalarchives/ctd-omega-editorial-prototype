...
router.post('/iteration-3/dashboard-action', function (req, res) {

  var action = req.body.action;

  if(action == 'add'){
    res.redirect('/iteration-3/search')
  } else {
    res.redirect('iteration-3/dashboard')
  }

})

router.post('/iteration-3/edit-set-action', function (req, res) {


  //var action = req.body.action;
  res.redirect('/iteration-3/edit-set')

})

router.post('/iteration-3/edit-record-action', function (req, res) {

 
  var action = req.body.action;

  if(action == 'save'){
    res.redirect('/iteration-3/confirmation-saved')
  } else if(action == 'add-creator'){
    res.redirect('/iteration-3/edit-record-add-creator')
  } else if(action == 'remove-creator'){
    res.redirect('/iteration-3/edit-record#select-creator')
  } else if(action == 'calculateDates'){
    res.redirect('/iteration-3/edit-record#covering-dates')
  } else {
    res.redirect('/iteration-3/confirmation-discarded')
  }

})


router.post('/iteration-3/searchResultsAction', function (req, res) {
...
router.post('/iteration-3/selectWorkSetAction', function (req, res) {

  var workSetAction = req.session.data['workSetAction']
  if (workSetAction == 'Add records and continue searching' || workSetAction == 'Create Work Set and search more records'){
    res.redirect('search-results')
  } else {
    res.redirect('view-work-set-added-message')
  }

})

router.post('/iteration-3/abandonWorkSetAction', function (req, res) {

  var abandonWorkSet = req.session.data['abandonWorkSet']
  if (abandonWorkSet == 'Yes'){
